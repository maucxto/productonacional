// Sistema de Barra para Producto Nacional
...
async function initializeBar() {
    try {
        ...
        console.log('Sistema de barra inicializado');
        
    } catch (error) {
        console.error('Error inicializando barra:', error);
...
async function loadOrders() {
    try {
        const response = await api.getOrdersForStation('bar');
        if (response.success) {
...
function setupWebSocketListeners() {
    // Nueva orden
    ws.on('bar_order', (data) => {
        console.log('New bar order:', data);
    ...
    // Orden lista
    ws.on('ready_bar', (data) => {
        playNotificationSound('order_ready');
        showToast(`¡Bebidas ${data.orderId} listas para mesa ${data.tableNumber}!`, 'success');
    });
...
function createOrderCard(order) {
    ...
    const isUrgent = elapsedTime > 15; // More than 15 minutes for drinks
    
    // Check for complex cocktails
    const hasComplexCocktails = barItems.some(item => {
        const product = db.getProductById(item.productId);
        return product && product.category === 'COCTELES' && product.prepTime > 3;
    });
    ...
    return `
        <div class="order-card bg-white rounded-2xl p-4 shadow-lg cursor-pointer ${isUrgent ? 'pulse-animation' : ''}" 
            ...
            <div class="space-y-2 mb-3">
                ${barItems.slice(0, 3).map(item => {
                    const product = db.getProductById(item.productId);
                    return `
                        <div class="flex items-center justify-between text-sm">
                            <div class="flex items-center">
                                <i class="fas ${getProductIcon(product)} text-xs mr-2 text-gold"></i>
                                <span class="font-medium">${product ? product.name : 'Bebida desconocida'}</span>
                            </div>
            ...
            <div class="flex items-center justify-between">
                <div class="text-xs text-gray-500">
                    ${totalItems} ${totalItems === 1 ? 'bebida' : 'bebidas'}
                </div>
                <div class="flex items-center space-x-2">
                    ${hasComplexCocktails ? `
                        <i class="fas fa-star text-amber-500 text-xs" title="Cocteles complejos"></i>
                    ` : ''}
...
function getProductIcon(product) {
    if (!product) return 'fa-glass-whiskey';
    ...
    return icons[product.category] || 'fa-glass-whiskey';
}
    ...
    content.innerHTML = `
        <div class="p-6">
            ...
            <div class="space-y-3 mb-6">
                ...
                ${barItems.map(item => {
                    const product = db.getProductById(item.productId);
                    const isComplex = product && product.category === 'COCTELES' && product.prepTime > 3;
                    
                    return `
                        <div class="flex items-center justify-between p-3 bg-white rounded-lg border ${isComplex ? 'border-amber-200 bg-amber-50' : ''}">
                            <div class="flex-1">
                                <div class="flex items-center mb-1">
                                    <i class="fas ${getProductIcon(product)} text-sm mr-2 text-gold"></i>
                                    <h4 class="font-semibold">${product ? product.name : 'Bebida desconocida'}</h4>
                                    ${isComplex ? `
                                        <i class="fas fa-star text-amber-500 text-xs ml-2" title="Coctel complejo"></i>
                                    ` : ''}
                                </div>
...
async function markAsReady(orderId) {
    try {
        ...
        if (response.success) {
            ...
            showToast('¡Bebidas listas!', 'success');
            playNotificationSound('order_ready');
        }
    } catch (error) {
...
function showNewOrderNotification(order) {
    ...
    if ('Notification' in window && Notification.permission === 'granted') {
        new Notification(`Nueva orden de bebidas - Mesa ${order.tableNumber}`, {
